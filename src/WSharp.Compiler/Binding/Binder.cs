...
public sealed class Binder
{
	...
	private BoundExpressionStatement BindExpressionStatement(ExpressionStatementSyntax syntax) =>
		new (syntax, this.BindExpression(syntax.Expression, true));

	private BoundLineStatements BindLineStatements(LineStatementsSyntax syntax)
	{
		var lineStatements = new List<BoundLineStatement>();

		foreach (var line in syntax.Lines)
		{
			lineStatements.Add(this.BindLineStatement(line));
		}

		return new BoundLineStatements(syntax, lineStatements);
	}

	private BoundLineStatement BindLineStatement(LineStatementSyntax syntax)
	{
		var lineNumber = (BigInteger)((LiteralExpressionSyntax)syntax.Number.Expression).Value!;

		if (!this.LineNumbers.Add(lineNumber))
		{
		...
		{
			this.doesStatementExistAfterDefer = this.deferWasInvoked is not null;
			boundLines.Add(this.BindStatement(lineStatement));
		}

		if (this.deferWasInvoked is not null && !this.doesStatementExistAfterDefer)
		{
			this.Diagnostics.ReportNoStatementsAfterDefer(this.deferWasInvoked.Location);
		}

		return new(syntax, boundLineNumber, boundLines);
	}

	private static BoundLiteralExpression BindLiteralExpression(LiteralExpressionSyntax syntax) =>
		new(syntax, syntax.Value ?? BigInteger.Zero);
	...
	public HashSet<BigInteger> LineNumbers { get; } = [];
#pragma warning disable CA1002 // Do not expose generic lists
	public List<(BigInteger, TextLocation)> LineNumberValidations { get; } = [];
