...
internal static class CompilationTests
{
	...
	private static void GetStatements(string code, Action<List<BoundStatement>> statements)
	{
		...
		{
			Assert.That(compiler.Diagnostics, Is.Empty, nameof(compiler.Diagnostics));
			Assert.That(compiler.Tree, Is.SameAs(tree), nameof(compiler.Tree));

			var lineStatements = (BoundLineStatements)compiler.Statement;
			Assert.That(lineStatements.LineStatements, Has.Count.EqualTo(1), nameof(lineStatements.LineStatements));
		...
		CompilationTests.GetStatements("1 print(\"hi\")", statements =>
			{
				using (Assert.EnterMultipleScope())
				{
					Assert.That(statements, Has.Count.EqualTo(1), nameof(statements.Count));
					...
					Assert.That(call.Arguments, Has.Length.EqualTo(1), nameof(call.Arguments));
					...
					Assert.That(call.Function.Parameters, Has.Length.EqualTo(1), nameof(call.Function.Parameters));
