...
namespace WSharp.Runtime.Tests
{
	public static class ExecutionEngineTests
	{
		...
		public static void CreateWhenGivenNullWriter() =>
			Assert.That(() => new ExecutionEngine(ImmutableArray.Create(new Line(1, 1, _ => { })), new Random(), null!, new StringReader(string.Empty)), 
				Throws.TypeOf<ArgumentNullException>());

		[Test]
		public static void CreateWhenGivenNullReader() =>
			Assert.That(() => new ExecutionEngine(ImmutableArray.Create(new Line(1, 1, _ => { })), new Random(), new StringWriter(), null!),
				Throws.TypeOf<ArgumentNullException>());
		...
		public static void CreateWhenGivenEmptyList()
		{
			...
			Assert.That(() => new ExecutionEngine(lines, new Random(), new StringWriter(), new StringReader(string.Empty)), Throws.TypeOf<ArgumentException>());
		}
		...
		public static void CreateWhenListContainsNullEntries()
		{
			...
			Assert.That(() => new ExecutionEngine(lines, new Random(), new StringWriter(), new StringReader(string.Empty)), Throws.TypeOf<ExecutionEngineLinesException>());
		}
		...
		public static void CallDoesLineExist(ulong identifier, ulong count, ulong identifierToSearch, bool expectedDoesLineExist)
		{
			...
			var engine = new ExecutionEngine(lines, new Random(), new StringWriter(), new StringReader(string.Empty));
		...
		public static void CallPrint()
		{
			...
			var writer = new StringWriter();
			var lines = ImmutableArray.Create(new Line(1, 3, _ => { }));
			var engine = new ExecutionEngine(lines, new Random(), writer, new StringReader(string.Empty));
